...
- Hoeveel energie je vannaar het net haalt  
...
if uploaded_file
    ...
    if not vereist.issubset(df.columns)
        ...
        for _, r in df_kwartier.iterrows()
            ...
            # Ontladen
            potentieel_ontladen = min(
                afname  (eff_ontladen  vertraging_ontladen + 1e-9),
        ...
        with tab2
            ...
            ax.fill_between(res[tijd], res[batterij_niveau (kWh)], alpha=0.3, label=Batterijniveau)
            ax.scatter(res[tijd], res[opladen (kWh)], s=5, label=Opladen)
            ax.scatter(res[tijd], res[ontladen (kWh)], s=5, label=Ontladen)
            ax.axhline(soc_max, linestyle='--', label=SoC max)
            ax.axhline(soc_min, linestyle='--', label=SoC min)
            ax.set_xlabel(Tijd)
            ax.set_ylabel(kWh)
            ax.legend()
            ax.grid(True)
        ...
        with tab3
            ...
            st.download_button(⬇️ Gedetailleerde kwartiertabel (CSV), csv_res, simulatie_resultaten.csv, textcsv)
            st.download_button(⬇️ Samenvatting (CSV), csv_summary, simulatie_samenvatting.csv, textcsv)
            st.download_button(⬇️ Maandelijkse winst (CSV), csv_maand, simulatie_maandwinst.csv, textcsv)
